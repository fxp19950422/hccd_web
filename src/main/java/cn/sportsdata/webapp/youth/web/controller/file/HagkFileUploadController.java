...
@Controller
@RequestMapping("/hagkFile")
public class HagkFileUploadController extends BaseController {
	private static Logger logger = Logger.getLogger(HagkFileUploadController.class);
	...
	public String fileUpload(HttpServletRequest request, @RequestParam MultipartFile uploadedFile) {
		try {
			...
			File createdFile = HAFileUtils.createNewAttachmentFile(createdFileName, loginVO.getId(), orgVO.getId(), HAFileUtils.PRODUCT_FILE_TYPE);
			uploadedFile.transferTo(createdFile);
