 ...
 *https://github.com/adriengibrat/jQuery-crop
 */
( function ( $, undefined ) {
		...
		, plugin  = function ( image, options ) {
			...
			this.$image
				...
				.on( 'load.' + namespace, function () {
					if ( img.naturalWidth === undefined || !img.naturalWidth) {
						if (self.$image.attr( 'src' ))
					...
					if(self.minPercent >= 1){  // img which has lower width or height than the viewport, disable control area
			...
			;
			if ( this.options.controls && this.options.slideControls === undefined){
				this.$frame
					.append( node( 'div', 'controls' )
						.append( node( 'span', 'text' ).append( this.options.controls ) )
						.append( node( 'a', 'zoomIn' ).on( 'click.' + namespace, $.proxy( this.zoomIn, this ) ).text('+') )
						.append( node( 'a', 'zoomOut' ).on( 'click.' + namespace, $.proxy( this.zoomOut, this ) ).text('-') )
					)
				;
			}else if( this.options.slideControls === true){
				this.$frame
					.append( node( 'div', 'controls' ).addClass("slide")
						.append( node( 'a', 'zoomOut' ).on( 'click.' + namespace, $.proxy( this.zoomOut, this ) ) )
						.append( node( 'span', 'slideContainer' ).append( '<input type="text" id="imgCropSlide" class="hide"><label class="hide" for="imgCropSlide">&nbsp;</label>' ) )
						.append( node( 'a', 'zoomIn' ).on( 'click.' + namespace, $.proxy( this.zoomIn, this ) ) )
					);
				$("a.cropZoomIn", this.$frame).attr("href","javascript:void(0);");
				$("a.cropZoomOut", this.$frame).attr("href","javascript:void(0);");
			}
			if(this.hideControls){
				this.$frame.find(".cropControls").addClass("hide");
			}
	...
	, {
		prototype : {
			...
			, zoomIn  : function () {
				var result = !! this.zoom( this.percent + 1 / ( this.options.zoom - 1 || 1 ) );
				if(this.options.zoomIn){
					this.options.zoomIn.apply(this);
				}
				return result;
			}
			, zoomOut : function () {
				...
				if(this.options.zoomOut){
					this.options.zoomOut.apply(this);
				}
				return result;
			}
			, update  : function () {
