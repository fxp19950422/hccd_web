		...
		function touchstart (e) {
			...
			if(e.originalEvent && typeof e.originalEvent.touches !== 'undefined'){
				...
				if (e.originalEvent.touches.length == 1) { //1 finger
					...
					//init pos
					self.startTouch.x = self.previousTouch.x = e.originalEvent.touches[0].pageX;
					self.startTouch.y = self.previousTouch.y = e.originalEvent.touches[0].pageY;
					
				} else if (e.originalEvent.touches.length == 2) { //two fingers
					
					self.isOneFingerGesture = false;
					
					if (e.originalEvent.touches[0].pageY > e.originalEvent.touches[1].pageY) {//0 is lower
						...
						self.startTouch.x = self.previousTouch.x = self.touches[1].pageX;
						self.startTouch.y = self.previousTouch.y = self.touches[1].pageY;
		...
		function touchmove(e) {
			...
			if (e.originalEvent && typeof e.originalEvent.touches !== 'undefined'){
				...
				if (e.originalEvent.touches.length == 1 || self.isOneFingerGesture) {//we ignore the second finger if we are already in movement
					...
					self.currentTouch.x = e.originalEvent.touches[0].pageX;
					self.currentTouch.y = e.originalEvent.touches[0].pageY;
					...
					if (e.originalEvent.touches[0].pageY > e.originalEvent.touches[1].pageY) {//0 is lower
						...
						self.currentTouch.x = e.originalEvent.touches[1].pageX;
						self.currentTouch.y = e.originalEvent.touches[1].pageY;
